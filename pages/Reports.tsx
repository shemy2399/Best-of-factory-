...
const ProgressBar: React.FC<{ label: string, value: number, max: number, color: string }> = ({ label, value, max, color }) => (
    <div className="space-y-2">
        ...
        <div className="w-full bg-gray-900 print:bg-gray-200 rounded-full h-3 overflow-hidden shadow-inner border border-gray-700/50 print:border-gray-300">
            <div 
                className="h-full rounded-full transition-all duration-[1200ms] ease-out shadow-lg" 
...
const DonutChart: React.FC<{ data: { label: string, value: number, color: string }[] }> = ({ data }) => {
  ...
  if (total === 0) return (
      <div className="flex items-center justify-center h-64 w-64 rounded-full border-4 border-gray-800 border-dashed text-gray-600 font-bold">
          NO DATA
      </div>
  );
    ...
    <div className="relative w-64 h-64 mx-auto print:w-48 print:h-48">
      <svg viewBox="0 0 32 32" className="w-full h-full transform -rotate-90 drop-shadow-2xl">
        {data.map((item, index) => {
          ...
          return (
            <circle key={index} r="16" cx="16" cy="16" fill="transparent" stroke={item.color} strokeWidth="6" strokeDasharray={dashArray} strokeDashoffset={dashOffset} className="transition-all duration-[1500ms] ease-in-out hover:opacity-90" />
          );
        })}
...
const ReportsPage: React.FC<ReportsPageProps> = ({ clinicLog = [], horses = [], globalBattalionFilter, monthlyArchives = [], onAddArchive, onDeleteArchive, onNavigateWithFilter }) => {
  ...
  const stats = useMemo(() => {
    const c = filteredHorses.reduce((acc, h) => {
    ...
    return [
      ...
      { label: 'مريض', value: c.sick || 0, color: '#ef4444' }, 
    ];
  }, [filteredHorses]);

  // تحليل الموقف الطبي للشهر الحالي
  const clinicStats = useMemo(() => {
    ...
    const thisMonthLogs = clinicLog.filter(log => {
      ...
      return d.getMonth() === currentMonth && d.getFullYear() === currentYear && horseIds.has(log.horseId);
    });
  ...
  const handleArchiveMonth = async () => {
    if (globalBattalionFilter === 'الكل') {
      alert('يرجى اختيار كتيبة محددة للأرشفة');
      return;
    }
    ...
    <div className="space-y-10 text-white pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 no-print">
        <div>
          <h1 className="text-4xl font-black tracking-tight flex items-center gap-3">
              <ReportsIcon className="w-10 h-10 text-amber-500" />
              التقارير والإحصائيات
          </h1>
          <p className="text-gray-400 mt-2 font-medium">تحليل البيانات والموقف الطبي لقطاع الخيالة.</p>
        </div>
        <div className="flex items-center gap-4">
            <button onClick={handleArchiveMonth} className="px-6 py-3 bg-gray-800 text-amber-500 font-bold rounded-xl hover:bg-gray-700 border border-gray-700 shadow-lg transition-all">
                أرشفة بيانات الشهر
            </button>
            <button onClick={() => window.print()} className="flex items-center px-8 py-3 bg-amber-500 text-white font-bold rounded-xl hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:grid-cols-2 print:gap-4">
          ...
          <div className="bg-gray-800 print:bg-white p-8 rounded-[2rem] shadow-2xl border border-gray-700 print:border-black flex flex-col items-center relative overflow-hidden">
              ...
              <h2 className="text-xl font-black mb-8 self-start flex items-center gap-3 print:text-black uppercase tracking-tighter w-full border-b border-gray-700 pb-4">
              ...
              <div className="grid grid-cols-3 gap-4 mt-8 w-full">
                  {stats.map(s => (
                      <div key={s.label} className="text-center p-3 bg-gray-900/50 print:bg-gray-50 rounded-xl border border-gray-700/50">
                          <p className="text-[10px] text-gray-500 print:text-black font-black mb-1 uppercase tracking-wider">{s.label}</p>
                          <p className="text-2xl font-black print:text-black" style={{ color: s.color }}>{s.value}</p>
                      </div>
                  ))}
              </div>
          </div>

          {/* تحليل الحالات الفعلي */}
          <div className="bg-gray-800 print:bg-white p-8 rounded-[2rem] shadow-2xl border border-gray-700 print:border-black relative overflow-hidden">
              ...
              <h2 className="text-xl font-black mb-8 flex items-center gap-3 print:text-black uppercase tracking-tighter w-full border-b border-gray-700 pb-4">
                  <span className="w-2 h-8 bg-blue-500 rounded-full"></span>
                  تحليل الحالات (الشهر الحالي)
              </h2>
              ...
              <div className="space-y-6">
                  ...
                  {clinicStats.total === 0 && (
                    <div className="p-4 bg-gray-900/50 rounded-xl text-center border border-gray-700/50 mt-4">
                        <p className="text-gray-400 text-sm font-medium">لا توجد حالات مسجلة في العيادة لهذا الشهر.</p>
                    </div>
                  )}
              </div>
          </div>
      ...
      <div className="bg-gray-800 print:bg-white p-8 rounded-[2rem] shadow-2xl border border-gray-700 print:border-black">
          <h2 className="text-xl font-black mb-8 flex items-center gap-3 print:text-black uppercase tracking-tighter border-b border-gray-700 pb-4">
              ...
              توزيع القوة حسب الأرسان (Bloodlines)
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {rasanDistribution.map(([name, count]) => (
                  <button 
                    key={name} 
                    onClick={() => onNavigateWithFilter(name)}
                    className="bg-gray-900/50 print:bg-gray-50 p-5 rounded-2xl text-center border border-gray-700 hover:border-cyan-500 hover:bg-gray-700 transition-all group"
                  >
                      <p className="text-[10px] text-gray-500 print:text-black font-black mb-2 group-hover:text-cyan-400 uppercase tracking-tighter truncate">{name}</p>
                      <p className="text-3xl font-black text-white print:text-black group-hover:text-white transition-colors">{count}</p>
                  </button>
              ))}
          </div>
      </div>

      {/* سجل الأرشيف التاريخي */}
      <div className="no-print space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {monthlyArchives.length > 0 ? monthlyArchives.map(archive => (
                  <div key={archive.id} className="bg-gray-800 p-6 rounded-[2rem] border border-gray-700 hover:border-purple-500 transition-all group relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start mb-4 relative z-10">
                          <div>
                              <h3 className="font-black text-lg text-white">{archive.monthLabel}</h3>
                              <p className="text-[10px] text-purple-400 font-bold uppercase tracking-widest">{archive.battalion}</p>
                          </div>
                          <button onClick={() => onDeleteArchive(archive.id)} className="text-gray-600 hover:text-red-500 transition-colors">
                              <TrashIcon className="w-5 h-5" />
                          </button>
                      </div>
                      <div className="grid grid-cols-2 gap-3 mt-4 relative z-10">
                          <div className="bg-gray-900/50 p-3 rounded-xl border border-gray-700/50">
                              <p className="text-[9px] text-gray-500 font-black mb-1">إجمالي القوة</p>
                              <p className="text-xl font-black text-white">{archive.totalCases}</p>
                          </div>
                          <div className="bg-gray-900/50 p-3 rounded-xl border border-gray-700/50">
                              <p className="text-[9px] text-gray-500 font-black mb-1">نسبة السليم</p>
                              <p className="text-xl font-black text-green-400">
                                  {archive.diagnoses.find(d => d.name === 'سليم')?.count || 0}
                              </p>
                          </div>
                      </div>
                  </div>
              )) : (
                  <div className="col-span-full p-10 text-center bg-gray-800/30 rounded-[2rem] border-2 border-dashed border-gray-700">
                      <p className="text-gray-500 font-bold">لا توجد سجلات مؤرشفة حتى الآن.</p>
                  </div>
              )}
          </div>
      </div>
      ...
      <div className="bg-gray-800 print:bg-white rounded-[2rem] shadow-2xl border border-gray-700 print:border-black overflow-hidden mt-8">
          <div className="p-6 border-b border-gray-700 print:border-black bg-gray-900/30 print:bg-gray-100">
              <h2 className="text-lg font-black print:text-black flex items-center gap-3 uppercase tracking-tighter">
                  ...
                  تدقيق الموقف الطبي (Audit Matrix)
              </h2>
          </div>
              ...
              <table className="min-w-full text-right">
                  <thead className="bg-gray-900/80 print:bg-gray-200 text-gray-400 print:text-black font-black uppercase tracking-widest text-[10px]">
                      <tr>
                          ...
                          <th className="px-8 py-4 text-center text-red-500">مريض</th>
                      ...
                      {BATTALIONS.map(bat => {
                          ...
                          return (
                            <tr key={bat} className="hover:bg-gray-700/30 transition-colors">
                                ...
                                <td className="px-8 py-4 text-center text-green-400 print:text-black">{bh.filter(h => h.status === 'healthy').length}</td>
                                <td className="px-8 py-4 text-center text-amber-400 print:text-black">{bh.filter(h => h.status === 'monitoring').length}</td>
                                <td className="px-8 py-4 text-center text-red-400 print:text-black">{bh.filter(h => h.status === 'sick').length}</td>
